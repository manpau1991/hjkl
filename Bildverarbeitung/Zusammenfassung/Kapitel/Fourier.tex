...
\begin{remark}[Eigenschaften der Fourier-Transformation und deren graphische Deutung] \leavevmode
  ...
  \begin{figure}[ht]
    ...
    HIER BEGINNT MEIN DAEMLICHER BEWEIS:

    \begin{equation}\label{eq:faltungsbeweis}
      \begin{split}
        (f * g)^{\wedge}(\xi)
        &\stackrel{\mathrm{def.}}= \int_\R (f*g)(t) e^{-i\xi t} \INT{t} \\
        &\stackrel{\mathrm{def.}}= \int_\R \int_\R f(t-x)g(x)\INT{x}\; e^{-i\xi t}\INT{t} \\
        &\stackrel{\mathrm{Tonelli}}= \int_\R \int_\R f(t-x)g(x)e^{-i\xi t}\INT{x}\INT{t} \\
        &\stackrel{l = t-x}= \int_\R \int_\R f(l)g(x)e^{-i\xi (l+x)}\INT{x}\INT{l} \\
        &\stackrel{\mathrm{Tonelli}}= \int_\R f(l)e^{-i\xi l}\INT{l} \int_\R g(x)e^{-i\xi x}\INT{x} \\
        &\stackrel{\mathrm{def.}}= \hat f(\xi) \hat g(\xi)
      \end{split}
    \end{equation}
...
%\begin{definition}[Fourierkoeffizienten und Fourierreihe]
%Zu $ f \in L_{1}(\T) $ sind die \emph{Fourierkoeffizienten} definiert als
%\[
%  f_{k} \coloneqq \frac{1}{2\pi} \int_{\T} f(t) e^{-ikt} \dif t, \qquad k \in \Z,
%\]
%und die \emph{Fourierreihe} zu $ f $ als
%\[
%  \mathcal{F}f \coloneqq \sum_{k \in \Z} f_{k} e^{ik\bullet}.
%\]
%\end{definition}
%
%\begin{remark}[Fourierkoeffizienten und Fourierreihe]
%Die Folge der Partialsummen $ (\mathcal{F}_{n} f)_{n \in \N} $ von $ \mathcal{F}f $ definiert durch
%\[
%  \mathcal{F}_{n} f \coloneqq \sum_{k = -n}^{n} f_{k} e^{ik\bullet}, \qquad n \in \N
%\]
%ist die Fourier-Transformation der Folge der Fourierkoeffizienten $ (f_{k})_{k \in \Z} $. Die 
%Operation
%\[
%  c \mapsto c^{\vee} \coloneqq \frac{1}{2\pi} c^{\wedge}(- \bullet)
%\]
%bezeichnet man als Inverse Fourier-Transformation zu einer Folge $ c \in l_{1}(\Z) $. Es ist nicht
%garantiert, dass die Fourierreihe von $ f $ konvergiert.
%\end{remark}