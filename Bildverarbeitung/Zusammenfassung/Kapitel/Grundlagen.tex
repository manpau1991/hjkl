...
\enquote{quadratsummierbar}). Leider gibt es hierfür keine so schöne geometrische Anschauung, da wir
...
Für die mathematisch Interessieren unter uns: $ L_{1}(\R) \cap L_{2}(\R) $ liegt 
dicht\footnote{Siehe dazu auch~\ref{seq:densityoflebesguespaces}} in
$ L_{2}(\R) $, und es gilt:
\[
...
Skalarprodukt\footnote{Achtung: Ein Skalarprodukt auf Lebesgue-Räumen ist nur auf $L_2$ definiert,
für alle anderen Lebesgue-Räume existiert kein Skalarprodukt.} definieren kann.

In $ l_{2}(\R) $ befinden sich übrigens auch nur Nullfolgen, da die Reihen zu den Folgen sonst 
wieder nicht konvergieren würden. Allerdings ist jetzt $ (1 / k)_{k \in \Z} \in l_{2}(\R)$.
\end{remark}
